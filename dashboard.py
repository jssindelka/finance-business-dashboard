...
def _gsheet():
    """Return the gspread Spreadsheet object (cached 15 min).
...
def _drive_find_folder(parent_id, name):
    """Find a folder by name under parent. Returns folder ID or None (cached 15 min)."""
...
def _auto_scan_changes():
    """Cached auto-scan for Drive invoice + expense changes on page load (10-min cache)."""
    changes = []
    _scan_errors = []
    try:
